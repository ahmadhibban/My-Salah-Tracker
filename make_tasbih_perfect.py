...
public class PremiumTasbihView extends LinearLayout {
    ...
    public PremiumTasbihView(Context context, boolean isDark, int accentColor) {
        ...
        // --- 2. Ultimate 3D Physical Button ---
        tapButton = new View(context);
        float density = context.getResources().getDisplayMetrics().density;
        int btnSize = (int) (70 * density); // Slightly larger for better tap area
        ...
        tapButton.setOnTouchListener((v, event) -> {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                // Slight scale down for mechanical feel
                ...
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    incrementCount();
                    vibrate(context, 35); // Slightly stronger haptic for mechanical feel
                }
                return true;
            }
            return false;
    ...
    private LayerDrawable createMechanicalButton(boolean isPressed) {
        ...
        // Housing/Bezel ring color (Plastic/Metallic look)
        ...
        // 2. Bezel (Outer Ring) - Gives the housing look from the image
        ...
        // 3. Button Face (The colored part)
        ...
        // Perfecting the 3D depth insets
        if (isPressed) {
            layerDrawable.setLayerInset(0, 2, 2, 0, 0); // Shadow shifts slightly
            layerDrawable.setLayerInset(1, 2, 2, 4, 4); // Bezel stays static
            layerDrawable.setLayerInset(2, 10, 10, 6, 6); // Button face pushed deep inside bezel
        } else {
            layerDrawable.setLayerInset(0, 0, 0, 6, 6); // Shadow drops bottom-right
            layerDrawable.setLayerInset(1, 0, 0, 8, 8); // Bezel raised
            layerDrawable.setLayerInset(2, 4, 4, 12, 12); // Button face popped out, thick on bottom-right
        }
...
def main():
    ...
    if target_dir:
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(java_code)
        print(f"✅ পারফেক্ট! নতুন মেকানিক্যাল ৩ডি লুকটি সেভ করা হয়েছে: {file_path}")
    else:
