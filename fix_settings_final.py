...
for r, d, f in os.walk('.'):
    if 'MainActivity.java' in f and 'build' not in r:
        ...
        # আগে যদি কোনো সেটিংস পপ-আপ থেকে থাকে, তা মুছে পরিষ্কার করা হচ্ছে
        c = re.sub(r'mr\.addImg\([^,]+,\s*(?:isDarkTheme\s*\?\s*"ic_sun"\s*:\s*"ic_moon"|"ic_sun"|"ic_moon").*?ad\.show\(\);\s*\}\s*\);', '', c, flags=re.DOTALL)
        c = re.sub(r'mr\.addImg\([^,]+,\s*"img_moon".*?ad\.show\(\);\s*\}\s*\);', '', c, flags=re.DOTALL)
        ...
        # নতুন আলাদা ঘর (Box) ও ডট (•) যুক্ত সেটিংস বসানো
...
mr.addImg(sp.getString("app_lang", "en").equals("bn") ? "তারিখ এডজাস্ট (আরবি/বাংলা)" : "Adjust Date (+/-)", "img_moon", new Runnable() {
    @Override public void run() {
        ...
        print("✅ ৫. সেটিংসের পপ-আপগুলো আলাদা ঘরে (Box) বসানো হয়েছে!")
        break
