...
public class FirebaseManager
{
    ...
    public void save(final String dateKey, final String prayerName, final String status)
    {
        ...
        new Thread(new Runnable() {
            @Override public void run()
            {
                try {
                    URL url =
                        new URL(DB_URL + safeEmail + "/" + dateKey + "_" + prayerName + ".json");
                    ...
                    sp.edit()
                        .putString("offline_q", q + dateKey + "|" + prayerName + "|" + status + ",")
                        .apply();
                    ...
                    androidx.work.WorkManager.getInstance(activity).enqueueUniqueWork(
                        "auto_sync", androidx.work.ExistingWorkPolicy.REPLACE, syncWork);
                }
            }
        }).start();
    }
    ...
    public void processOfflineQueue(
        final Runnable onStart, final Runnable onSuccess, final Runnable onFail)
    {
        String q = sp.getString("offline_q", "");
        if (!q.isEmpty() && !sp.getString("user_email", "").isEmpty()) {
            ...
            final String safeEmail =
                sp.getString("user_email", "").replace(".", "_dot_").replace("@", "_at_");
                ...
                @Override public void run()
                {
                    ...
                    for (String item : items) {
                        ...
                        if (parts.length == 3) {
                            try {
                                URL url = new URL(
                                    DB_URL + safeEmail + "/" + parts[0] + "_" + parts[1] + ".json");
                                HttpURLConnection con = (HttpURLConnection) url.openConnection();
                                con.setConnectTimeout(5000);
                                con.setReadTimeout(5000);
                                con.setRequestMethod("PUT");
                    ...
                    if (jsonResult != null && !jsonResult.equals("null")
                        ...
                        try {
                            ...
                            while (keys.hasNext()) {
                                ...
                                editor.putString(k,
                                    val);  // ব্যাকআপ হিসেবে SharedPreferences এ সেভ
                                ...
                                if (k.length() >= 10 && k.matches("\\d{4}-\\d{2}-\\d{2}.*")) {
                                    ...
                                    if (type.equals("Fajr"))
                                        record.fajr = val;
                                    else if (type.equals("Dhuhr"))
                                        record.dhuhr = val;
                                    else if (type.equals("Asr"))
                                        record.asr = val;
                                    else if (type.equals("Maghrib"))
                                        record.maghrib = val;
                                    else if (type.equals("Isha"))
                                        record.isha = val;
                                    else if (type.equals("Witr"))
                                        record.witr = val;
                                }
                            }
