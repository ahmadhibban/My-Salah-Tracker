...
public interface SalahDao {
    ...
    void insertRecord(SalahRecord record);

    // ডেটা আপডেট করবে (যখন ইউজার টিক চিহ্ন দেবে)
    @Update
    void updateRecord(SalahRecord record);
    ...
    @Query("SELECT SUM(fajr_qaza + dhuhr_qaza + asr_qaza + maghrib_qaza + isha_qaza + witr_qaza) FROM salah_records")
    int getTotalPendingQaza();
}