...
for r, d, f in os.walk('.'):
    if 'MainActivity.java' in f and 'build' not in r:
        ...
        # 1. হিজরি তারিখের সাইজ ছোট করা
        c = re.sub(r'dHijri\.setTextSize\(\s*\d+\s*\);', 'dHijri.setTextSize(14);', c)

        # 2. কাস্টম পপ-আপ (কালো টাইটেল এবং বিন্দুযুক্ত লিস্ট)
        old_settings_regex = r'// --- SETTINGS NEW START ---.*?// --- SETTINGS NEW END ---'
        ...
        if '// --- SETTINGS NEW START ---' in c:
            c = re.sub(old_settings_regex, new_settings, c, flags=re.DOTALL)
            with open(p, 'w', encoding='utf-8') as f: f.write(c)
            print("✅ MainActivity Patched (Custom Design Matching Perfectly)")
